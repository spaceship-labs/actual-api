...
describe('Payment service', function() {
  describe('addPayment', function() {
    it('should throw an error when is a card payment and doesnt have a terminal associated', async function() {
      ...
      expect(
        PaymentService.addPayment(params, req)
      ).to.eventually.be.rejectedWith(
        'Es necesario asignar una terminal a este tipo de pago'
      );
    });
    ...
    it('should throw an error when ewallet doesnt have enougth founds', async () => {
      const req = {};
      await EwalletConfiguration.create({
        id: 'ewalletConfig.id.1',
        exchangeRate: 20,
      });
      const ewallet = await Ewallet.create({
        id: 'ewallet.id.1',
        cardNumber: '12345678901234',
        amount: 200,
        Client: 'client.id.1',
      });
      const params = { type: 'ewallet', ewallet: ewallet.id };
      expect(
        PaymentService.addPayment(params, req)
      ).to.eventually.be.rejectedWith(
        'Fondos insuficientes en monedero electronico'
      );
    });
  });
  ...
  it('should calculate payments total', async function() {
    ...
    const result = PaymentService.calculatePaymentsTotal(
      payments,
      exchangeRate
    );
  ...
  it('should calculate payments total for payments group 1', async function() {
    ...
    const exchangeRate = 18.5;
    const result = PaymentService.calculatePaymentsTotalPg1(
      payments,
      exchangeRate
    );
  ...
  it('should calculate correctly a usd payment', async function() {
    ...
    const exchangeRate = 18.2;
    expect(PaymentService.calculateUSDPayment(payment, exchangeRate)).to.equal(
      1820
    );
  });
  ...
  it('should show only methods groups with group 1 for a client with special discount', async function() {
    ...
    const result = PaymentService.filterMethodsGroupsForDiscountClients(
      methodGroups
    );
  ...
  it('should add the credit method group to the method groups', async function() {
    ...
    const creditMethod = _.findWhere(paymentGroup1.methods, {
      type: PaymentService.types.CLIENT_CREDIT,
    });
  ...
  it('should add the single payment terminal method group to the method groups', async function() {
    ...
    expect(result).to.be.an('array');
    const paymentGroup1 = paymentGroups[0];
    const singlePaymentMethod = _.findWhere(paymentGroup1.methods, {
      type: PaymentService.types.SINGLE_PAYMENT_TERMINAL,
    });
    expect(singlePaymentMethod).to.be.an('object');
    expect(singlePaymentMethod.type).to.be.equal(
      PaymentService.types.SINGLE_PAYMENT_TERMINAL
    );
  });
  ...
  describe('mapStatusType', function() {
    it('should return the correct label for canceled status type', function() {
      ...
      expect(PaymentService.mapStatusType(payment.status)).to.be.equal(
        'Cancelado'
      );
    });
