...
module.exports = {
  ...
  applyFilters,
  applyOrFilters,
  applySlowMovementQuery,
  applySpotlightQuery,
  applyStockRangesQuery,
  applySocietiesQuery,
  areFiltersApplied,
  getDiscountPriceKeyByStoreCode,
  getMultiIntersection,
  getPriceQuery,
  getProductsByCategories,
  getProductsByCategory,
  getProductsByFilterValue,
  getProductsByGroup,
  getPromotionsQuery,
  getSortValueBySortOption,
  hashToArray,
  populateProductsIdsToPromotions,
  promotionCronJobSearch,
  queryIdsProducts,
  queryTerms,
...
function getProductsByCategories(categoriesIds, options) {
  ...
  var relationsArray = [];
  options = options || {};
  return Product_ProductCategory.find({ productCategory: categoriesIds }).then(
    function(relations) {
      relationsHash = getProductRelationsHash(
        relations,
        'product',
        'productCategory'
      );
      ...
      if (options.applyIntersection) {
        ...
        relationsArray = getRelationsWithCategories(
          relationsArray,
          categoriesIds
        );
      }
...
function getProductsByFilterValue(filtervaluesIds) {
  var relationsHash = {};
  var relationsArray = [];
  return Product_ProductFilterValue.find({
    productfiltervalue: filtervaluesIds,
  }).then(function(relations) {
    relationsHash = getProductRelationsHash(
      relations,
      'product',
...
function promotionCronJobSearch(opts) {
  ...
  return getProductsByCategories(categories, {
    ...
    .then(function(idProducts) {
      if (
        areFiltersApplied(categories, filtervalues, groups) &&
        idProducts.length > 0
      ) {
